...
- `app/api/health/route.ts` — `force-dynamic`, returns `{ status: 'ok' | 'stale' | 'cold', ageSeconds, lastSuccessAt, countries, uptimeSeconds, warming }`. **Always returns HTTP 200** so Railway/Docker rollout probes don't block during a cold start — the `status` body field is the operator signal. When `status === 'cold'`, the handler fires a fire-and-forget warm-up (`fetchDashboardData → setCached`, dedupe via module-level `warmingPromise`) so the cache fills within a few seconds of first probe. `stale` (>1h old) keeps streaming but is visible to operators. Wired to the Docker `HEALTHCHECK` and Railway `healthcheckPath`.
